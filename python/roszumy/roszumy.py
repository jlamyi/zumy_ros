...
#        return retu
    ...
    def ros_cmd(self, data):
        self.rlock.acquire()
        self.m_left.cmd(-data.linear.x)
        self.m_right.cmd(data.linear.y)
        self.rlock.release()

     def run(self):
     	 rospy.init_node('zumy_listener', anonymous=True)
         rospy.Subscriber("zumy_cmd",Twist,self.ros_cmd)
